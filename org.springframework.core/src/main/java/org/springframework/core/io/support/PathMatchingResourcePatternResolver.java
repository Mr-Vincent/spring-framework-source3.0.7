	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			...
			if (getPathMatcher().isPattern(locationPattern.substring(prefixEnd))) {
				// a file pattern 文件路径名
				return findPathMatchingResources(locationPattern);
			}
			else {
				// a single resource with the given name
				// 只有文件名
				// 先拿到resourceLoader 再去拿resource 这里的resourceLoader是DefaultResourceLoader 同样也实现了ResourceLoader接口
				// 他妈的真是绕
				// loader到底将字符串转化为什么样的resource 也是在DefaultResourceLoader完成的
				// 具体返回 new ClassPathContextResource(path, getClassLoader())这个实例
